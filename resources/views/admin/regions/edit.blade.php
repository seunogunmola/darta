                    ...
                    <div class="border p-4 rounded">
                        ...
                        <form action="{{ route('regions.update',$region->id) }}" method="post">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <input name="region_name" required value="{{ old('region_name',$region->region_name) }}" type="text" class="form-control" id="region_name" placeholder="Enter State Name">
                                </div>
                            </div>
                            <div class="row mb-3">
                                <label for="inputEnterYourName" class="col-sm-3 col-form-label">State Abbreviation</label>
                                <div class="col-sm-9">
                                    <input name="region_abbreviation" required value="{{ old('region_abbreviation',$region->region_abbreviation) }}" type="text" class="form-control" id="region_abbreviation" placeholder="Enter State Abbreviation">
                                </div>
                            </div>
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <select name="state_id" id="state_id" required class="form-control">
                                        ...
                                        @foreach ($states as $state)                                            
                                            <option {{ $region->state_id == $state->id ? "selected" : ""}} value="{{ $state->id }}">{{ $state->state_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>                            
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <select name="status" id="status" required class="form-control">
                                        <option value="">Select Option</option>
                                        <option {{ $region->status == 1 ?  "selected" : "" }} value="1">Enabled</option>
                                        <option {{ $region->status == 0 ?  "selected" : "" }} value="0">Disabled</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row">
